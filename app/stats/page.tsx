...
function Page() {
  ...
  return loading ? (
    ...
    <div className="flex w-full flex-col gap-12 text-[#FFF0DD] justify-start lg:mt-12 mb-10 mt-10">
      ...
      */}
      <div className="flex flex-col min-[1300px]:flex-row justify-center gap-4">
        <VolumeCart
      ...
      <table className="overflow-y-scroll border-separate border-spacing-y-1 lg:text-xl text-xs w-full">
        <tbody className="overflow-y-scroll block table-fixed w-full mx-auto h-[auto]">
          <tr className="bg-[rgba(26,29,36,0.80)] backdrop-blur-[52px] rounded-[48px] w-[80%] text-[#FFF0DD] ">
            ...
            <td className=" hidden min-[1300px]:table-cell py-3 w-[12%]">From Amount</td>
            <td className=" hidden min-[1300px]:table-cell py-3 w-[12%]">To Amount</td>
            <td className=" hidden min-[1300px]:table-cell  py-3 w-[15%]">To Token</td>
            <td className=" hidden min-[1300px]:table-cell  py-3 w-[15%]">From Token</td>
          ...
          {statsData?.last20tx?.map((item: any, index: number) => {
            return (
              ...
              >
                ...
                <td className="lg:text-base hidden min-[1300px]:table-cell w-[12%] ">
                  {item?.outUsd.toFixed(2)}$
                </td>
                <td className="hidden min-[1300px]:table-cell lg:text-base w-[15%]">
                  {item?.inUsd.toFixed(2)}$
                </td>
                <td className="hidden min-[1300px]:table-cell  w-[15%] lg:text-base">
                ...
                <td className=" text-right w-[10%] lg:text-base pr-2 py-2">
                  {timestampToDate(item?.tx?.timestamp * 1000)}
                </td>
              </tr>
            );
          })}
