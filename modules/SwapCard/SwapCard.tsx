...
const SwapCard: React.FC<Props> = () => {
  ...
  const [bestRouteData, setBestRouteData] = useState<BestRouteData | undefined>(
    undefined
  );
  ...
  const [translateRouteCard, setTranslateRouteCard] = useState({
    x: 0,
    y: 0,
    xie: 0,
    yie: 0,
          ...
          const convertedToken = new ERC20Token(
            ...
            token.address.find(
              (address: { chainId: number; address: string }) =>
                address.chainId === ChainId.SCROLL_MAINNET
            ).address,
            token.decimals,
            token.symbol,
            token.name,
  ...
  useEffect(() => {
    const getChildlist = async () => {
      const response = await axios.post("/api/getChildlist", {
        chainId: ChainId.SCROLL_MAINNET,
      });
  ...
  const generateBestRouteData = async (
    tokenIn: Currency,
    tokenOut: Currency,
    amountIn: string
  ) => {
    ...
    const amountInParsed = BigInt(
      String(ethers.utils.parseUnits(amountIn, tokenIn?.wrapped?.decimals))
    );
    ...
    if (tokenFrom?.symbol == "WETH" && tokenTo?.symbol == "ETH") {
      ...
      setTimeout(() => {
        ...
        const swapValue =
          tokenIn?.isNative || tokenFrom?.wrapped?.address === tokens![1].wrapped?.address
      ...
      try {
        ...
        setReceiveAmount(
          Number(
            ethers.utils.formatUnits(responseParsed.amountOut, tokenOut?.decimals)
          ).toFixed(6)
        );
        setMinimumReceived(
          Number(
            ethers.utils.formatUnits(responseParsed.minAmountOut, tokenOut?.decimals)
          ).toFixed(6)
        );
        setPriceImpact(
          String(
            responseParsed.priceImpact < 0.01
              ? 0.01
              : responseParsed.priceImpact.toFixed(2)
          )
        );

        const swapValue =
          tokenIn?.isNative ||
          responseParsed.swapParams[0].tokenIn === tokens![1].wrapped?.address
            ? ethers.utils.parseEther(amountIn)
            : BigInt(0);
        setSwapValue(String(swapValue));
        ...
        for (let i = 0; i < responseParsed.swapParams.length; i++) {
          if (i == 0) {
            swapParamsConvertedToRoutes.push([]);
            swapParamsConvertedToRoutes[swapParamsConvertedToRoutes.length - 1].push(
              responseParsed.swapParams[i]
            );
          } else {
        ...
        for (let i = 0; i < responseParsed.swapParams.length; i++) {
          for (let j = 0; j < swapParamsConvertedToRoutes.length; j++) {
            for (let k = 0; k < swapParamsConvertedToRoutes[j].length; k++) {
              if (
                responseParsed.swapParams[i] !== swapParamsConvertedToRoutes[j][k + 1] &&
                responseParsed.swapParams[i].tokenIn ===
                  swapParamsConvertedToRoutes[j][k].tokenIn &&
                responseParsed.swapParams[i].tokenOut ===
                  swapParamsConvertedToRoutes[j][k].tokenOut &&
                responseParsed.swapParams[i].swapType !==
                  swapParamsConvertedToRoutes[j][k].swapType
              ) {
        ...
        let finalRoute: SwapParams = {
          poolAddress:
            responseParsed.swapParams[responseParsed.swapParams.length - 1].poolAddress,
          tokenIn:
            responseParsed.swapParams[responseParsed.swapParams.length - 1].tokenOut,
          tokenOut:
            responseParsed.swapParams[responseParsed.swapParams.length - 1].tokenOut,
          amountIn:
            responseParsed.swapParams[responseParsed.swapParams.length - 1].amountIn,
          amountOutMin:
            responseParsed.swapParams[responseParsed.swapParams.length - 1].amountOutMin,
          ...
          deadline:
            responseParsed.swapParams[responseParsed.swapParams.length - 1].deadline,
          isStable:
            responseParsed.swapParams[responseParsed.swapParams.length - 1].isStable,
          convertToNative:
            responseParsed.swapParams[responseParsed.swapParams.length - 1]
              .convertToNative,
          swapType:
            responseParsed.swapParams[responseParsed.swapParams.length - 1].swapType,
        };
  ...
  useEffect(() => {
    ...
    if (tokenFrom!.symbol == "ETH") {
      ...
      if (!!allowance && !!swapAmount) {
        if (
          Number(
            ethers.utils.formatUnits(String(allowance), tokenFrom?.wrapped?.decimals)
          ) >= Number(swapAmount)
        ) {
          setApproved(true);
        } else {
  ...
  const handleClickInputPercent = async (percent: number) => {
    ...
    if (
      ...
      setSwapAmount(
        ((parseFloat(balance) * (percent === 100 ? percent - 2.3 : percent)) / 100)
          .toFixed(6)
          .toString()
      );
      await generateBestRouteData(
        tokenFrom!,
        tokenTo!,
        ((parseFloat(balance) * (percent === 100 ? percent - 2.3 : percent)) / 100)
          .toFixed(6)
      ...
      setPercentage(percent);
      setSwapAmount(
        ((parseFloat(balance) * (percent === 100 ? percent - 0.00001 : percent)) / 100)
          .toFixed(6)
          .toString()
      );
      await generateBestRouteData(
  ...
  const { config: configApprove } = usePrepareContractWrite({
    ...
    args: [
      ...
      ethers.utils.parseUnits(
        Number(swapAmount ? swapAmount : "0").toFixed(tokenFrom?.wrapped?.decimals),
        tokenFrom?.wrapped?.decimals
      ),
    ],
    enabled:
      !!contracts &&
  ...
  const { data: dataEthDeposit, writeAsync: onEthDeposit } =
    useContractWrite(configEthDeposit);
  ...
  const { data: dataWethWithdraw, writeAsync: onWethWithdraw } =
    useContractWrite(configWethWithdraw);
              ...
              if (tokenFrom?.symbol == "ETH" && tokenTo?.symbol == "WETH") {
                ...
                if (!onSwap) {
                  ...
                  if (
                    ...
                    return toast("You don't have enough balance for this transaction.");
                  }
                  if (
                    error?.message.includes("Execution reverted for an unknown reason.")
                  ) {
                    return toast("Execution reverted for an unknown reason.");
                  }
  ...
  return (
    <div className="relative w-full md:min-h-[60rem] xs:min-h-[50rem] min-h-[40rem] flex lg:flex-row flex-col lg:justify-center justify-start lg:items-start items-center gap-0">
      <motion.div
        ...
        className={
          "lg:absolute relative h-fit xl:w-[520px] lg:w-[480px] md:w-[400px] sm:w-[360px] w-full xs:max-w-full max-w-[320px] py-2 gap-2 flex flex-col mx-auto " +
          (!(
            showRouteModal &&
            routes &&
            bestRouteData &&
      ...
      >
        <div
          className={`relative w-full h-full gap-4 flex-1 flex justify-between flex-col`}
        >
          <div className="relative w-full flex flex-col mb-2">
            <div className="w-full z-[1] flex flex-col bg-[rgba(26,29,36,0.80)] mb-1 backdrop-blur-[52px] xs:rounded-[48px] rounded-[32px] lg:px-8 md:px-6 xs:px-4 px-2 xs:py-4 py-2 !pb-12">
              <div className="flex justify-between items-center mt-4 sm:px-4 px-6">
                <span className="text-white xl:text-xl lg:text-lg md:text-md xs:text-base text-sm">
                  From
                </span>
                {balanceFrom && (
                ...
                <div className="flex flex-row flex-wrap justify-center items-center xl:gap-6 gap-2">
                  {percentageButtons.map((val, index) => (
                    <div
                      className={`${
                        percentage === val
                          ? "scale-[1.1] bg-white bg-opacity-5"
                          : "scale-100"
                      } ${
                        balanceFrom &&
                        balanceTo &&
                        Number(toFixedValue(balanceFrom!.formatted, 6)) > 0
                          ? ""
                          : "pointer-events-none opacity-50"
                      } group cursor-pointer bg-black select-none xl:px-2 xl:py-2 px-1 py-2 lg:w-[5rem] sm:w-[4.5rem] w-[3.25rem] rounded-full bg-opacity-[0.15] flex flex-col text-center sm:text-sm text-xs transition-all duration-150 hover:cursor-pointer hover:bg-white hover:bg-opacity-5`}
            ...
            <div className="w-full flex flex-col bg-[rgba(26,29,36,0.80)] backdrop-blur-[52px] xs:rounded-[48px] rounded-[32px] lg:px-8 md:px-6 xs:px-4 px-2 xs:py-4 py-2 xs:pb-12 !pb-6">
              ...
              <div className="flex flex-col w-full lg:mt-6 md:mt-5 sm:mt-4 mt-3 sm:mb-2 xs:mb-0 mb-5">
                ...
                <div
                  className={
                    "flex justify-center items-center w-full md:mt-6 mt-4 md:mb-3 mb-0 " +
                    (Number(swapAmount) == 0 ||
                    swapAmount == undefined ||
                    bestRouteData == undefined
                      ? ""
                      : "")
                  }
                >
                  <Button
                    ...
                    className={
                      "select-none uppercase w-full xs:mx-0 mx-2 md:p-4 sm:p-3 p-2 rounded-xl xl:text-xl sm:text-lg text-md font-semibold " +
                      (swapButtonDisableHandler() ? "opacity-50 pointer-events-none" : "")
                    }
                    onClick={swapButtonOnClickHandler}
                  >
                    {(!approved && isLoadingApprove) ||
                    isLoadingSwap ||
                    isLoadingEthDeposit ||
                    isLoadingWethWithdraw ? (
                      ...
                      chain?.id !== ChainId.SCROLL_MAINNET ? (
                        "Switch Network"
                      ) : approved ? (
                        "Swap"
                      ) : (
                ...
                {isMoreInformationVisibleAll && (
                  <div
                    ...
                    className={`flex flex-col select-none justify-between md:p-5 p-3 md:mx-0 xs:mx-1 mx-2 md:mt-3 mt-4 bg-[#121419] bg-opacity-30 rounded-xl gap-1 hover:bg-white hover:bg-opacity-5 hover:cursor-pointer ${""}`}
                  >
                    <div className="flex flex-row flex-wrap justify-between items-center w-full">
                      <span className="text-white xs:text-base text-sm">
                        More Information
                      </span>
                      <FaChevronDown
                        className={
                          "text-white transition-all duration-200 " +
                          (isMoreInformationVisible ? "rotate-180" : "")
                        }
                      />
                    </div>
                          ...
                          {isLoadingReceiveAmount ? (
                            ...
                            <span className="text-white xs:text-base text-sm">
                              {Number(minimumReceived) > 0
                                ? minimumReceived +
                                  " " +
                                  (tokenTo?.symbol ? tokenTo?.symbol : "TOKEN")
                                : "- " + (tokenTo?.symbol ? tokenTo?.symbol : "TOKEN")}
                            </span>
                          )}
                        </div>
          ...
          {showFrom && (
            <TokenModal
              onSelectToken={async (token: any) => {
                ...
                if (!!swapAmount && !!tokenTo)
                  await generateBestRouteData(token, tokenTo!, swapAmount!);
          ...
          {showTo && (
            <TokenModal
              onSelectToken={async (token: any) => {
                ...
                if (!!swapAmount && !!tokenFrom)
                  await generateBestRouteData(tokenFrom!, token, swapAmount!);
