...
function RouteCard({ onCloseModal, routes, routesAndSpaces, childlist, tokens, routePercentages, amountOuts, tokenFrom, translateRouteCard }: Props) {
  ...
  return (
      <motion.div
        initial={{ opacity: 0, translateX: translateRouteCard.xie + "%", translateY: translateRouteCard.yie + "%", scale: 0.9 }}
        animate={{ opacity: 1, translateX: translateRouteCard.x + "%", translateY: translateRouteCard.y + "%", scale: 1 }}
        exit={{ opacity: 0, translateX: translateRouteCard.xie + "%", translateY: translateRouteCard.yie + "%", scale: 0.9 }}
          ...
          <div className="md:px-8 px-0 md:min-w-fit md:scale-100 scale-75">
            <div className="flex flex-row justify-between items-start w-full gap-6">
                  ...
                  <div className="flex flex-col justify-center items-center min-w-[6.25rem] max-w-[13.75rem]">
                    <div className="flex flex-col justify-center items-center w-full">
                      <div className="flex flex-row justify-center items-center rounded-full w-[3.25rem] h-[3.25rem] overflow-clip">
                        <Image onClick={() => window.open("https://scrollscan.com/")} src={String(tokens?.find(token => (token?.wrapped.address == tokenFrom?.wrapped.address))?.logo!)} width={48} height={48} alt="" className="bg-black bg-opacity-[0.15] p-2 hover:cursor-pointer w-full h-full"/> 
                      </div>
              ...
              {routesAndSpaces?.map((route, index) => (
                route.length > 0 ? (
                  <div key={"route-" + index} className="flex flex-col justify-center items-center min-w-fit max-w-[13.75rem]">
                    <div className="flex flex-col justify-center items-center w-full">
                      <div className="flex flex-row justify-center items-center rounded-full w-[3.25rem] h-[3.25rem] overflow-clip">
                        <Image onClick={() => window.open("https://scrollscan.com/token/" + route[0].tokenIn)} src={String(tokens?.find(token => (token?.symbol != "ETH") && (token?.wrapped.address == route[0].tokenIn))?.logo!)} width={48} height={48} alt="" className="bg-black bg-opacity-[0.15] p-2 hover:cursor-pointer w-full h-full"/> 
                      </div>
